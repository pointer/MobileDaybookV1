 ...
 export default {
   ...
   methods: {
     onSend: function () {
       ...
       let urlNode = 'http://localhost/jsonapi/daybook_duty_taking'
       window.fetch(urlToken)
      .then((response) => response.text())
      .then((token) => {
        let newNode = {
          'data': {
            type: 'node--daybook_duty_taking',
            'attributes': {
              'status': true,
              'title': self.username,
              'field_dbk_duty_taking_agent': self.username,
              'field_dbk_duty_taking_agent_id': uid,
              'field_dbk_duty_taking_end': self.punchDate,
              ...
              'field_dbk_duty_taking_site': self.site,
              'field_dbk_duty_taking_start': self.punchDate
            }
          }
        }
        let fetchData = {
